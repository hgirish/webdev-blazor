...
public class BlogApiJsonDirectAccess : IBlogApi
{
        ...
        IOptions<BlogApiJsonDirectAccessSetting> option)
    {
        ...
        if (!Directory.Exists(blogPostsFolderPath))
        {
            Directory.CreateDirectory(blogPostsFolderPath);
        }
        var categoriesFolderPath =
            $@"{_settings.DataPath}\{_settings.CategoriesFolder}";
        if (!Directory.Exists(categoriesFolderPath))
        {
            Directory.CreateDirectory(categoriesFolderPath);
        }
        var tagsFolderPath = 
            $@"{_settings.DataPath}\{_settings.TagsFolder}";
    ...
    public async Task<BlogPost?> SaveBlogPostAsync(BlogPost item)
    {
        ...
        await SaveAsync<BlogPost>(
            _blogPosts, _settings.BlogPostsFolder,
            $"{item.Id}.json", item);
        return item;
    }
    ...
    public async Task<Category?> SaveCategoryAsync(Category item)
    {
        ...
        await SaveAsync<Category>(_categories, 
